...
rawtxnfrom::rawtxnfrom()
    : org::minima::system::commands::Command(
          "rawtxnfrom",
          "[inputs:] [outputs:] [scripts:] (state:) - Create an unsigned transaction from a set of inputs, outputs, scripts and state") {
}

std::vector<std::string> rawtxnfrom::getValidParams() const {
    return {"inputs","outputs","scripts","state"};
}

std::string rawtxnfrom::getFullHelp() const {
    return std::string("\nrawtxnfrom\n")
        + "\n"
        + "Create an unsigned transaction with a list of input coinids, output JSON coins, scripts and state.\n"
        + "\n"
        + "inputs:\n"
        + "    A JSONArray of coinids to use as inputs.\n"
        + "\n"
        + "outputs:\n"
        + "    A JSONArray of output JSON coins with an address, amount and optional tokenid and storestate.\n"
        + "\n"
        + "scripts:\n"
        + "    A JSONArray of scripts to add to the inputs, in the same order.\n"
        + "\n"
        + "state: (optional)\n"
        + "    The JSON state.\n"
        + "\n"
...
std::unique_ptr<JSONObject> rawtxnfrom::runCommand() {
    ...
    for (const std::any& ainput : inelems) {
        std::string coinid = anyToStringValue(ainput);
    ...
    // Add Scripts
    const auto& screlems = scripts->elements();
    for (const std::any& ascript : screlems) {
        std::string script = anyToStringValue(ascript);
...
std::shared_ptr<JSONObject> rawtxnfrom::runCommandSingle(const std::string& zCommand) {
...
std::shared_ptr<JSONObject> rawtxnfrom::anyToJSONObject(const std::any& a) {
    ...
    throw CommandException("JSONArray element is not a JSONObject");
}

std::string rawtxnfrom::anyToStringValue(const std::any& a) {
    // Try std::string
    ...
    // Try a JSONObject - mirror Java's toString()
    if (a.type() == typeid(std::shared_ptr<JSONObject>)) {
        auto ptr = std::any_cast<std::shared_ptr<JSONObject>>(a);
        if (ptr) {
            return ptr->toString();
        }
        throw CommandException("Null JSONObject shared_ptr in JSONArray");
    }
    if (a.type() == typeid(JSONObject*)) {
        JSONObject* op = std::any_cast<JSONObject*>(a);
        if (!op) {
            throw CommandException("Null JSONObject* in JSONArray");
        }
        return op->toString();
    }
    if (a.type() == typeid(JSONObject)) {
        return std::any_cast<const JSONObject&>(a).toString();
    }

    throw CommandException("JSONArray element is not a string");
}

void rawtxnfrom::skipWS(const std::string& s, std::size_t& i) {
    while (i < s.size() && std::isspace(static_cast<unsigned char>(s[i]))) {
        ++i;
    }
}

std::string rawtxnfrom::parseJSONStringQuoted(const std::string& s, std::size_t& i) {
    // Expects s[i] == '"'
    if (i >= s.size() || s[i] != '"') {
...
std::string rawtxnfrom::parseJSONValueToken(const std::string& s, std::size_t& i) {
...
std::vector<std::pair<int, std::string>> rawtxnfrom::parseFlatStateObject(const JSONObject& obj) {
...
org::minima::system::commands::Command* rawtxnfrom::getFunction() {
