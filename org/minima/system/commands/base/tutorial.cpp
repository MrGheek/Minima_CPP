...
std::string tutorial::getFullHelp() const {
    return std::string("\ntutorial\n")
        + "\n"
        + "Show the complete Grammar for Minima KISSVM scripting.\n"
        + "\n"
...
std::unique_ptr<org::minima::utils::json::JSONObject> tutorial::runCommand() {
    // Base helper to create a reply object
    std::unique_ptr<org::minima::utils::json::JSONObject> ret = getJSONReply();

    const std::string tutorial_text =
        std::string("\n")
    ...
    ret->put("response", tutorial_text);

    return ret;
}

org::minima::system::commands::Command* tutorial::getFunction() {
    return new tutorial();
}
